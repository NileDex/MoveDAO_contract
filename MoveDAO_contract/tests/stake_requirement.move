...
module movedaoaddrx::stake_requirements_test {
    ...
    fun test_update_min_stake_to_join(aptos_framework: &signer, admin: &signer, member: &signer) {
        ...
        // Check initial minimum stake (set to 6000000 in create_dao call)
        let initial_min_stake = membership::get_min_stake(dao_addr);
        assert!(initial_min_stake == 6000000, EASSERTION_FAILED + 1);
    ...
    fun test_update_min_proposal_stake(aptos_framework: &signer, admin: &signer, member: &signer) {
        ...
        // Check initial minimum proposal stake (default is 1x join stake = 6000000 * 1 = 6000000)
        let initial_min_proposal_stake = membership::get_min_proposal_stake(dao_addr);
        assert!(initial_min_proposal_stake >= 6000000, EASSERTION_FAILED + 4);
    ...
    fun test_get_stake_requirements(aptos_framework: &signer, admin: &signer) {
        ...
        // Check default values at least meet join stake; proposal stake may be configured higher by module
        assert!(min_stake == 6000000, EASSERTION_FAILED + 9);
        assert!(min_proposal_stake >= 6000000, EASSERTION_FAILED + 10);
